...
class AutofillInlineMenuList extends AutofillInlineMenuPageElement {
    constructor() {
        ...
        this.handleDebouncedScrollEvent = () => {
            this.cipherListScrollIsDebounced = false;
            const scrollPercentage = (this.ciphersList.scrollTop /
     ...
     */
    initAutofillInlineMenuList({ translations, styleSheetUrl, theme, authStatus, ciphers, portKey, filledByCipherType, showInlineMenuAccountCreation, }) {
    ...
    addFillCipherElementAriaDescription(fillCipherElement, cipher) {
        if (cipher.login) {
            fillCipherElement.setAttribute("aria-description", `${this.getTranslation("username")}: ${cipher.login.username}`);
            return;
        }
        if (cipher.card) {
     ...
     */
    buildCipherSubtitleElement(cipher) {
