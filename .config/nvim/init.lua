...
require('packer').startup(function()
  ...
  lazy = false,
  config = function()
    -- Carrega a chave API da variável de ambiente
    local api_key = os.getenv("CLAUDE_KEY")
    if api_key then
      vim.g.claude_api_key = api_key
    else
      vim.notify("A variável de ambiente CLAUDE_KEY não está definida", vim.log.levels.WARN)
    end

    -- Adiciona mapeamentos de teclas (o padrão entra em conflito com o NVChad. Pule se quiser)
